 ...
 *        of each conversion so that next conversion result can be written into the result register.</b>
 ...
 * @code
  ...
  int main(void)
  {
     ...
     //Get the result
     AdcResult |=  ADCCH001_GetFastResult((ADCCH001_HandleType*)&ADCCH001_Handle0);
     while(1);
     return 0;
  }
 * @endcode
...
/**
 * @brief If result register is 0 then Indicates the setting of the external
 * multiplexer controlled by the ADC EMUX controller. This setting corresponds to the current result. 
 * In case of other result register different from result register 0 is used, then this information is not relevant.
 * @param[in] HandlePtr pointer to the Instance variable<BR>
 * @param[out] EmuxPtr is External multiplexer setting
 * @return <b>uint16_t</b><BR>
 *
 *
 * <b>Reentrant: yes</b><BR>
 ...
 * @code
  ...
  int main(void)
  {
    uint32_t Emux_Status=0;
    uint8_t Emuxch;
    DAVE_Init();//ADC001_Init is called within DAVE_Init
 
    //Get the External multiplexer converting channel number.
    Emux_Status |=  ADCCH001_GetEmux((ADCCH001_HandleType*)&ADCCH001_Handle0, &Emuxch);
    while(1);
    return 0;
  } 
 * @endcode
 */
status_t ADCCH001_GetEmux(const ADCCH001_HandleType* HandlePtr, uint8_t* EmuxPtr);
