...
/**
 * @brief When receive fifo is enabled, this function reads out the USIC receive
 * FIFO Buffer, word by word, until there is data in the buffer or reached the 
 * input count value. 
 ...
 * {
 *   UART001_ReadDataMultiple(&UART001_Handle1,Readdata1,1);
 *   Readdata1++;
 * }
 * @endcode<BR> </p>
 *
...
/**
 * @brief When receive fifo is enabled, this function reads out the USIC receive
 * FIFO Buffer, byte by byte, until there is data in the buffer or reached the 
 * input count value. 
 ...
 *  void RecvFifoHandler()
 * {
 * 	 UART001_ReadDataBytes(&UART001_Handle0,&Readdata,11);
 * }
 * // Read data byte from standard buffer when fifo is disabled.
 * void RecvStdBuffHandler()
 * {
 *   UART001_ReadDataBytes(&UART001_Handle1,Readdata1,1);
 *   Readdata1++;
 * }
 * @endcode<BR> </p>
 * 
 ...
 * transmit buffer register, word by word, until the FIFO buffer is full or 
 ...
 *  {
 *  	while(!UART001_WriteDataMultiple(&UART001_Handle1, &data1, 1));
 *  }
 *  while(1);
 *  return 0;
 * }
...
/**
 * @brief When fifo is enabled, this function writes data to the USIC FIFO 
 * transmit buffer register, byte by byte, until the FIFO buffer is full or 
 * reached the Count value.
 ...
 *  	status = UART001_WriteDataBytes(&UART001_Handle0, &data, 11);
 *  if(status == 11)
 *  {
 * 	 //data transmitted
 *  } 
 ...
 *  for(count=0; count<11; count++)
 *  {
 *  	while(!UART001_WriteDataBytes(&UART001_Handle0, &data1, 1));
 *  }
 *  while(1);
 *  return 0;
 * }
